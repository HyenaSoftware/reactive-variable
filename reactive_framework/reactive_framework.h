...
namespace reactive_framework
{
	...
	template<class T, class I = std::string> class rv
	{
		...
		static_assert(!std::is_same<T, void>::value, "Type T can not be void.");
	public:
	...
	template<class... Ts, class... Is, class TRAIT = merge_trait<
			typename Utility::first_of<Ts...>::type,
			typename Utility::first_of<Is...>::type
